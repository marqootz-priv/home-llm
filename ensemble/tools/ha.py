"""Home Assistant REST API tool for Ensemble."""
import httpx
...
def control_home(
    ...
    action: "get_state" | "call_service" | "list_entities"
    """
    try:
        if action == "get_state":
            if not entity_id:
            ...
            with httpx.Client(timeout=15.0) as client:
                ...
                r.raise_for_status()
                states = r.json()
                entities = [{"entity_id": s.get("entity_id"), "state": s.get("state")} for s in states]
                return {"ok": True, "entities": entities}
        else:
            return {"ok": False, "error": f"Unknown action: {action}. Use get_state, call_service, or list_entities."}
